...
class LlamaManager:
    ...
    def _is_asset_compatible(self, asset_name: str) -> Tuple[bool, Optional[str]]:
        ...
        if any(token in {"arm", "arm64", "aarch64"} for token in tokens):
            return (
                False,
                "ARM builds are not supported by the container (expects x86_64)",
            )
    ...
    def _select_release_asset(
        ...
        if asset_id is not None:
            ...
            if not record:
                raise Exception(
                    f"Selected artifact (id={asset_id}) not found in release {tag_name}"
                )
            metadata = record["metadata"]
            if not metadata.get("compatible", False):
                reason = (
                    metadata.get("compatibility_reason")
                    or "Artifact is marked as incompatible"
                )
                raise Exception(
                    f"Selected artifact is not compatible with this container: {reason}"
                )
    ...
    def _resolve_release_asset(
        self, tag_name: str, asset_id: Optional[int] = None
    ) -> Dict:
        release = self._fetch_release(tag_name)
        compatible_assets, skipped_assets, asset_lookup = self._collect_release_assets(
            release
        )
        selected_metadata, binary_asset = self._select_release_asset(
            tag_name, asset_id, compatible_assets, skipped_assets, asset_lookup
        )
    ...
    def get_release_assets(self, tag_name: str) -> Dict:
        ...
        return {
            ...
            "skipped_assets": skipped_assets,
            "default_asset_id": (
                default_metadata.get("id") if default_metadata else None
            ),
        }

    def get_release_install_preview(
        self, tag_name: str, asset_id: Optional[int] = None
    ) -> Dict:
        ...
        try:
            ...
            if expected_size:
                try:
                    ...
                    if (
                        abs(downloaded_size - expected_size) > 2 * 1024 * 1024
                    ):  # 2 MB tolerance
                        logger.warning(
            ...
            if lowered_name.endswith(".zip"):
                ...
                raise Exception(
                    f"Unsupported archive format for {binary_asset['name']}"
                )
        ...
        try:
            ...
            if build_config.enable_cuda:
                cuda_available, cuda_root, cuda_error = (
                    self._check_cuda_toolkit_available()
                )
                ...
                if cmake_process.returncode != 0:
                    ...
                    if build_config.enable_cuda and (
                        ...
                        if validated_cuda_root:
                            ...
                            cuda_ver = (
                                self._get_cuda_version(nvcc_path)
                                if os.path.exists(nvcc_path)
                                else None
                            )
