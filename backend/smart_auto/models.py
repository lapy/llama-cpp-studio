...
class SystemResources:
    ...
    compute_capabilities: List[float] = field(default_factory=list)  # Pre-parsed compute capabilities
    
    @classmethod
    def from_gpu_info(cls, gpu_info: Dict[str, Any], cpu_memory: Tuple[float, float, float], 
        ...
        return cls(
            ...
            available_vram_gb=sum(gpu.get('memory', {}).get('free', 0) for gpu in gpus) / (1024**3) if gpus else 0.0,
