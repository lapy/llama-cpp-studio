...
def ctx_tokens_budget_greedy(model_size_gb: float, available_cpu_ram_gb: float, reserve_overhead_gb: float = None) -> int:
...
def calculate_kv_cache_size(
    ...
    quant_factor_v = get_kv_cache_quant_factor(cache_type_v) if cache_type_v else quant_factor_k
    
    if embedding_length > 0 and layer_count > 0:
        # Calculate bytes per element for K and V cache
        bytes_per_k = quant_factor_k * 4  # Convert factor to actual bytes (f32=4, f16=2, etc.)
    ...
    base_kv_cache_bytes = int(ctx_size * kv_cache_per_token * parallel * KV_CACHE_OPTIMIZATION_FACTOR)
...
def estimate_vram_usage(model: Model, config: Dict[str, Any], gpu_info: Dict[str, Any], 
                        metadata: Optional[ModelMetadata] = None, usage_mode: str = "single_user") -> Dict[str, Any]:
    ...
    try:
        ...
        kv_cache_bytes = calculate_kv_cache_size(
            ctx_size, parallel, total_layers, embedding_length,
            attention_head_count, attention_head_count_kv,
            cache_type_k, cache_type_v, usage_mode=usage_mode
        )
        ...
        fits_in_gpu = (n_gpu_layers == 0) or (estimated_vram <= max(0, total_free_vram * VRAM_SAFETY_MARGIN))
...
def estimate_ram_usage(model: Model, config: Dict[str, Any], metadata: Optional[ModelMetadata] = None, usage_mode: str = "single_user") -> Dict[str, Any]:
    ...
    try:
        ...
        if embedding_length > 0:
            ...
            intermediate_ram = int(batch_size * embedding_length * bytes_per_element * BATCH_INTERMEDIATE_FACTOR)
            
            # QKV projections are also temporary and reused
            qkv_ram = int(batch_size * 3 * embedding_length * bytes_per_element * BATCH_QKV_FACTOR)
        ...
        kv_cache_savings = int(ctx_size * parallel * kv_cache_per_token * (1 - (QUANTIZATION_AVERAGE_FACTOR * quant_factor_k + QUANTIZATION_AVERAGE_FACTOR * quant_factor_v)))
