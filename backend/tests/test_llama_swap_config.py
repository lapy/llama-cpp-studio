...
def test_normalize_swap_env_and_gguf_user_env_ld_split():
    ...
    um, uld = llama_swap_config._gguf_user_env_and_ld_suffix(
...
def test_yaml_uses_stable_id_and_routing_alias(monkeypatch, tmp_path):
    binary_path = tmp_path / "llama-server"
    binary_path.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
    binary_path.chmod(0o755)
    model_path = tmp_path / "model.gguf"
        ...
        all_models=[
            {
                ...
                "config": {
                    "engine": "llama_cpp",
                    "engines": {"llama_cpp": {"model_alias": "my-app"}},
                },
            }
        ],
    )
    ...
    assert "my-app" not in doc["models"]
...
def test_normalize_set_params_by_id_and_filters_block():
    ...
    filters, aliases = llama_swap_config._swap_model_filters_and_param_aliases(
        "my-model.q4_k_m", raw
    )
    assert filters == {
        "setParamsByID": {
...
def test_preview_handles_missing_proxy_and_missing_runtime(monkeypatch):
    monkeypatch.setattr("backend.data_store.resolve_llama_swap_id", lambda model: "")
    no_proxy = llama_swap_config.preview_llama_swap_command_for_model({"config": {}})
    assert no_proxy["ok"] is False
    assert no_proxy["error"] == "Model has no llama-swap id"

    monkeypatch.setattr("backend.data_store.resolve_llama_swap_id", lambda model: "proxy")
    monkeypatch.setattr(
        llama_swap_config,
        "get_active_binary_path_for_engine",
