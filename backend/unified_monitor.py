...
class UnifiedMonitor:
    ...
    def mark_model_ready(self, model_name: str):
        ...
        if model_name in self._models_loading:
            load_time = (datetime.utcnow() - self._models_loading[model_name]).total_seconds()
            del self._models_loading[model_name]
            logger.info(f"Model '{model_name}' is now ready (loaded in {load_time:.1f}s)")
    ...
    async def broadcast_model_event(self, event_type: str, model_name: str, details: Dict[str, Any] = None):
    ...
    def get_loading_models(self) -> Dict[str, Any]:
        ...
        for model_name, start_time in self._models_loading.items():
            ...
            if elapsed > self._loading_timeout:
                ...
                logger.warning(f"Model '{model_name}' loading timeout ({elapsed:.0f}s > {self._loading_timeout}s)")
            else:
                loading[model_name] = {
            ...
            try:
                ...
                for model in external_models:
                    ...
                    if previous_state != state:
                        logger.info(f"Model '{model_name}' state changed: {previous_state} → {state}")
                        ...
                        if state == 'loading':
                            ...
                            if model_name in self._models_loading:
                                load_time = (datetime.utcnow() - self._models_loading[model_name]).total_seconds()
                                logger.info(f"Model '{model_name}' ready after {load_time:.1f}s")
                                del self._models_loading[model_name]
                ...
                for prev_model in list(self._previous_model_states.keys()):
                    if prev_model not in current_model_names:
                        logger.info(f"Model '{prev_model}' stopped (removed from llama-swap)")
                ...
                logger.debug(f"Failed to poll /running (llama-swap may be starting): {e}")
