...
class GGUFReader:
    ...
    def __init__(self, file_path: str):
        ...
        self.tensor_data_start_offset: int = (
            0  # Track where tensor data begins (after alignment)
        )
    ...
    def _read_bytes(self, size: int) -> bytes:
        ...
        if self._offset + size > len(self._mm):
            raise EOFError(
                f"Attempted to read {size} bytes at offset {self._offset}, but file ended."
            )
    ...
    def _unpack(self, fmt: str, size: int) -> Tuple[Any, ...]:
        ...
        try:
            ...
            raise ValueError(
                f"Struct unpack failed at offset {self._offset - size}: {e}"
            )
    ...
    def _parse_header(self):
        ...
        logger.debug(
            f"Header Parsed: v{self.version}, Tensors={self.tensor_count}, KV={self.kv_count}"
        )
    ...
    def _read_string(self) -> str:
        ...
        if length > 10 * 1024 * 1024:
            raise ValueError(
                f"Security Alert: String length {length} exceeds safety limit (10MB)."
            )
        ...
        if value_type in self._TYPE_MAP:
            ...
            if value_type == GGUFValueType.BOOL:
                if result not in (0, 1):
                    raise ValueError(
                        f"Invalid BOOL value at offset {self._offset - size}: {result}. Must be 0 or 1."
                    )
                return bool(result)
    ...
    def _read_array(self, depth: int = 0) -> List[Any]:
        ...
        if depth > MAX_DEPTH:
            raise ValueError(
                f"Array recursion depth {depth} exceeds maximum allowed depth {MAX_DEPTH}. Possible DoS attack."
            )
        ...
        if count > (2**63):  # Sanity check for count itself
            raise ValueError(
                f"Array count {count} is unreasonably large. Possible integer overflow attack."
            )
        ...
        if item_type_id in self._TYPE_MAP:
            ...
            try:
                ...
                raise ValueError(
                    f"Array size calculation overflow: count={count}, size={size}"
                )

            # Additional sanity check
            if total_bytes > len(self._mm):
                raise ValueError(
                    f"Array claims {total_bytes} bytes, but file is only {len(self._mm)} bytes."
                )

            # Security: Pre-check file bounds
            if self._offset + total_bytes > len(self._mm):
                raise EOFError(
                    f"Array claims {total_bytes} bytes, but file is too short."
                )
            ...
            if count > 1_000_000:
                logger.debug(
                    f"Large array detected ({count} items). Switching to iterative reading."
                )
    ...
    def _read_array_iterative(
        self, item_type_id: int, count: int, depth: int = 0
    ) -> List[Any]:
        """
    ...
    def get_tensor_data(self, tensor_name: str) -> bytes:
        ...
        if tensor_name not in self.tensors:
            raise KeyError(
                f"Tensor '{tensor_name}' not found. Available tensors: {list(self.tensors.keys())[:10]}..."
            )

        if not self._mm:
            raise RuntimeError(
                "GGUFReader is not open. Use as context manager: 'with GGUFReader(path) as reader:'"
            )
        ...
        if absolute_offset > len(self._mm):
            raise ValueError(
                f"Tensor '{tensor_name}' offset {absolute_offset} exceeds file size {len(self._mm)}"
            )
        ...
        if estimated_size > max_available:
            logger.warning(
                f"Tensor '{tensor_name}' estimated size {estimated_size} exceeds available {max_available}. Using available bytes."
            )
...
def _extract_moe_info(metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in experts_used_keys:
        if key in metadata:
            ...
            if isinstance(value, (int, float)) and value > 0:
                ...
                is_moe = True
                logger.debug(
                    f"Found experts_used_count from key '{key}': {experts_used_count}"
                )
                break
    ...
    if is_moe and experts_used_count == 0 and final_expert_count > 0:
        ...
        logger.debug(
            f"Using default experts_used_count: {experts_used_count} for expert_count: {final_expert_count}"
        )
    ...
    if is_moe:
        logger.info(
            f"MoE detected: {final_expert_count} experts, {experts_used_count} active per token"
        )
    ...
    try:
        ...
        with GGUFReader(file_path) as reader:
            ...
            attention_head_count_kv = (
                ...
                or metadata.get(
                    "glm4.attention_head_count_kv"
                )  # Alternative GLM4 format
            ...
            return {
                "layer_count": (
                    int(effective_layer_count) if effective_layer_count else 0
                ),
                ...
                "attention_head_count": (
                    int(attention_head_count) if attention_head_count else 0
                ),
                "attention_head_count_kv": (
                    int(attention_head_count_kv) if attention_head_count_kv else 0
                ),
        ...
        logger.error(
            f"Failed to read GGUF metadata from {file_path}: {e}", exc_info=True
        )
        return None
...
def _extract_context_length(metadata: Dict[str, Any]) -> int:
        ...
        if key in metadata:
            ...
            if isinstance(ctx_len, (int, float)) and ctx_len > 0:
                ...
                if ctx_int > MAX_REASONABLE_CONTEXT:
                    logger.warning(
                        f"Unreasonably large context length detected from key '{key}': {ctx_len}, skipping"
                    )
                    continue
                logger.info(
                    f"Found context length candidate: {ctx_len} from key: {key}"
                )
    ...
    if detected_contexts:
        ...
        if len(set(detected_contexts)) > 1:
            logger.debug(
                f"Multiple context lengths detected {detected_contexts}, using max={max_ctx}"
            )
        return max_ctx
    ...
    available_keys = [
        ...
        if any(term in k.lower() for term in ["context", "length", "seq", "position"])
    ]
    logger.warning(
...
def _extract_parameter_count(metadata: Dict[str, Any]) -> Optional[str]:
    ...
    for key in param_keys:
        if key in metadata:
            ...
            if isinstance(param_count, (int, float)) and param_count > 0:
                logger.info(
                    f"Found parameter count candidate: {param_count} from key: {key}"
                )
                ...
                if param_str and param_str[-1].upper() in ("B", "M", "K"):
                    try:
                        ...
                        if float_part > 0:
                            logger.info(
                                f"Found pre-formatted parameter count: {param_str} from key: {key}"
                            )
                            # Return directly if already formatted (but normalize case)
                            return (
                                param_str.upper()
                                if param_str[-1].isupper()
                                else param_str
                            )
    ...
    available_keys = [
        k
        for k in metadata.keys()
        if any(term in k.lower() for term in ["param", "parameter"])
    ]
    if available_keys:
        logger.debug(
...
def _extract_layer_count(metadata: Dict[str, Any]) -> int:
    ...
    for key in layer_keys:
        if key in metadata:
            ...
            if isinstance(layer_count, (int, float)):
                logger.info(
                    f"Found layer count candidate: {layer_count} from key: {key}"
                )
    ...
    if detected_layers:
        ...
        if len(set(detected_layers)) > 1:
            logger.info(
                f"Multiple layer counts detected {detected_layers}, using max={max_layers}"
            )
        else:
    ...
    if "qwen" in architecture:
        ...
        embedding_length = metadata.get("qwen3moe.embedding_length") or metadata.get(
            "qwen.embedding_length", 0
        )
        if embedding_length:
            ...
            logger.debug(
                f"Estimated Qwen layer count: {estimated_layers} for embedding_length: {embedding_length}"
            )
    ...
    if "llama" in architecture:
        ...
        if vocab_size > 0 and embedding_length > 0:
            ...
            logger.debug(
                f"Estimated layer count: {estimated_layers} for architecture: {architecture}"
            )
            return estimated_layers

    # Log available keys for debugging
    logger.debug(
        f"Available metadata keys for layer detection: {[k for k in metadata.keys() if 'block' in k or 'layer' in k or 'block_count' in k]}"
    )
