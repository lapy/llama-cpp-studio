      ...
      <div class="task-detail-header">
        ...
        <div class="task-detail-meta">
          <Button
            ...
            outlined
            size="small"
            :loading="stopTaskId === task.task_id"
...
const hasDismissibleTasks = computed(() => {
  ...
  return filteredTasks.value.some((t) => t.status !== 'running')
})
...
function dismissAllFinished() {
  const ids = filteredTasks.value.filter((t) => t.status !== 'running').map((t) => t.task_id)
...
.task-detail-item.status-failed {
